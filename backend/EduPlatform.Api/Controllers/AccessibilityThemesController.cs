...
public class AccessibilityThemesController : ControllerBase
{
    ...
    {
        var q = _db.Set<AccessibilityTheme>().AsNoTracking().AsQueryable();
    ...
    public async Task<IActionResult> Create([FromBody] AccessibilityThemeDto dto)
    {
        ...
        if (cat == null) return BadRequest("Categoria inválida.");
    ...
    {
        var t = await _db.Set<AccessibilityTheme>().FindAsync(id);
        if (t == null) return NotFound();
        _db.Set<AccessibilityTheme>().Remove(t);
        await _db.SaveChangesAsync();
        return NoContent();
    }
}
